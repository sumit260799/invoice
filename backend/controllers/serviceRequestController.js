...
//       req.body;
//     const { name: userName, email: userEmail, role } = req.user;
...
const allocateForQuotation = async (req, res, next) => {
  ...
  try {
    ...
    if (!(role === 'billingManager' || role === 'admin')) {
    ...
    } else if (role === 'billingManager' || role === 'admin') {
      ...
      //     return res.status(400).json({ message: 'Invalid action. Use "Allocation" or "Reallocation".' });
      // }
      ...
      // if (billingRequest.quoteStatus == 'BillingPending' && billingRequest.quoteStatus == 'Billed' || (billingRequest.brStatus == 'PendingforInvoiceAllocation' && billingRequest.brStatus == 'InvoicingInProgress')) {
      //     return res.status(400).json({
      //         message: `Cannot ${action.toLowerCase()} for quoteStatus: ${billingRequest.quoteStatus}`,
      //     });
      // }
      ...
      if (
        ...
        //     message: `Cannot ${action.toLowerCase()} for quoteStatus: ${billingRequest.quoteStatus}`,
        // });
        //Below logic will be executed for brStatus == 'OnHold'/'Rejected'
        // Update Billing Request
        if (action === 'AllocationforQuotation') {
          billingRequest.allocatedTo_name = name;
          billingRequest.allocatedTo_email = email;
          billingRequest.billingProgressStatus = 'QuotationInProgress';
          billingRequest.allocatedAt = new Date();
          ...
          await ChangeLog.findOneAndUpdate({
            ...
            newValue: action,
            remarks: remarks || 'Initial allocation for Quotation',
          });

          await sendAllocationEmail(
            email,
            'Quotation Allocation Notification',
            `Hello ${name},\n\nYou have been allocated for billing request.\nBilling Request ID: ${serviceRequestId}\nRemarks: ${
              remarks || 'Allocation for Quotation'
            }\n\nYou have been allocated by: ${fullName}<${email}>\n\nRegards,\nYour Team`
          );
        } else if (action === 'ReallocationforQuotation') {
          billingRequest.reallocatedTo_name = name;
          billingRequest.reallocatedTo_email = email;
          billingRequest.billingProgressStatus = 'QuotationInProgress';
          billingRequest.reallocatedAt = new Date();
          await billingRequest.save();
          await ChangeLog.findOneAndUpdate({
            serviceRequestId,
            // newStatus,
            changeType: 'allocationStatus',
            newValue: action,
            remarks: remarks || 'Reallocation for Quotation',
          });

          await sendAllocationEmail(
            email,
            'Quotation Allocation Notification',
            `Hello ${name},\n\nYou have been allocated to a new quotation.\nBilling Request ID: ${serviceRequestId}\nRemarks: ${
              remarks || 'Allocation for Quotation'
            }\n\nYou have been allocated by: ${fullName}<${email}>\n\nRegards,\nYour Team`
          );
        } else res.status(501).json({ message: `Not Possible` });
        // await billingRequest.save();
      ...
      // if ((billingRequest.quoteStatus == 'PendingRelease' && billingRequest.quoteStatus == 'ApprovalPending') || (billingRequest.brStatus == 'PendingForQuotationAllocation' && billingRequest.brStatus == 'QuotationInProgress')) {
      //     return res.status(400).json({
      //         message: `Cannot ${action.toLowerCase()} for quoteStatus: ${billingRequest.quoteStatus}`,
      //     });
      // }
      ...
      if (
        ...
        if (action === 'AllocationforInvoice') {
          ...
          await ChangeLog.findOneAndUpdate({
            ...
            changeType: 'allocationStatus',
            newValue: action,
            remarks: remarks || 'Initial allocation for Invoice',
          });

          await sendAllocationEmail(
            email,
            'Invoice Allocation Notification',
            `Hello ${name},\n\nYou have been allocated to a new Invoicing.\nBilling Request ID: ${serviceRequestId}\nRemarks: ${
          ...
          await ChangeLog.findOneAndUpdate({
            ...
            changeType: 'allocationStatus',
            newValue: action,
            remarks: remarks || 'Reallocation for Invoice',
          });
